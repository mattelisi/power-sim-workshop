...
- **Defining effect sizes in a GLMM.**  
  We define the effect size in terms of the population-level SD across participant, so we multiply the standardised effect size by the standard deviation $\Delta_{\text{PSE}} = d \times \text{SD}_{\text{PSE}}$, which gives something that is conceptually similar to Cohen's $d$. Note that as we define effect size in SD units of latent PSE (between-person SD) does not include variability due to e.g. "measurement" error (which depends on task design, number of trials, etc.), and because of this the effect size is more conservative than an a true Cohen's $d$ of the same magnitude.
...
simulate_dd <- function(
  ...
  # we scale the effect size according to the SD across people

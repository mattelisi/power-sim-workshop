...
sim_diary <- function(
    ...
    # Outcome Y structure
    beta0   = 0,         # intercept
    beta_w  = 0.20,      # within-person effect of person-mean-centered X (x_cwc) on Y
    beta_b  = 0.20,      # between-person effect of child mean X (x_bar) on Y
    ...
    sigma_e = 0.80,      # residual SD (day-level noise)
    rho_e_ar1 = 0.2,     # AR(1) correlation in residuals across consecutive days
  ...
  #
  # AR(1) idea: today's deviation partly "carries over" from yesterday.
  #   out[t] = r * out[t-1] + innovation[t]
  #
  # If r > 0, adjacent days are positively correlated.
  # If r = 0, the series is just independent noise.
  #
  # We want the *marginal* SD of out[t] to be approximately 's' (at stationarity).
  # For a stationary AR(1), Var(out) = Var(innovation) / (1 - r^2).
  # So we set SD(innovation) = s * sqrt(1 - r^2).
  r_ar1 <- function(n, s, r){
    innov_sd <- s * sqrt(1 - r^2)
    innov <- rnorm(n, mean = 0, sd = innov_sd)
    
    out <- numeric(n)
    out[1] <- rnorm(1, mean = 0, sd = s)  # start near the stationary distribution
    for (t in 2:n) {
  ...
  # --- Build a balanced panel: each child measured each day ---
  ...
  # This makes X "sticky" across adjacent days within each child.
  df <- df %>%
    group_by(id) %>%
    mutate(x_within = r_ar1(n_days, sd_within_x, rho_x_ar1)) %>%
    ungroup() %>%
    mutate(
      x     = x_bar + x_within,   # observed X = person mean + daily deviation
      x_cwc = x - x_bar           # cluster-mean centered X (within-person component)
    )
  
  # Optional: standardize predictors for convenience (keeps coefficients in SD units)
  df <- df %>%
    mutate(
  ...
  # --- Day-level residuals e (optionally AR(1) within child) ---
  # This captures "extra" day-to-day autocorrelation in Y not explained by X.
  df <- df %>%
    group_by(id) %>%
    mutate(e = r_ar1(n_days, sigma_e, rho_e_ar1)) %>%
    ungroup()
  ...
  df <- df %>%
    mutate(
      y = beta0 + u0 +
        (beta_w + u1) * x_cwc +   # within-person effect with random slope
        beta_b * x_bar +          # between-person effect
        weekday_eff +
        e
    )
  
  # Optional: standardize outcome
  df <- df %>% mutate(y = as.numeric(scale(y)))
  
  # --- Missingness: day-level MCAR only ---
  # Each row is independently dropped with probability mcar_rate.
  if (mcar_rate > 0) {
...
fit_once <- function(df, weekday_fx = TRUE) {
  ...
  # We always include x_cwc and x_bar.
  # Weekday fixed effects are optional.
  if (weekday_fx) {
...
power_grid <- function(
  ...
  # Preallocate a list to store results from each simulation
...
# Scenario: modest within-person effect (beta_w = 0.20 SD), ICC ~ ~ tau0^2 / (tau0^2 + sigma_e^2)
# Keep random slope small; mild AR(1); 5% MCAR
quick_A <- power_grid(
  n_sims = 200,
  n_child = 50,
  n_days = 7,
